...
import { interactiveClasName } from "../styles";
...
export const Interactive = c<Props>(({ onMove, onKey, ...rest }) => {
  ...
  return html`<div ${spread(rest)} @touchstart=${handleMoveStart} @mousedown=${handleMoveStart}
  class=${interactiveClasName} ref=${ref(container)} @keydown=${handleKeyDown} tabindex="0" role="slider"
  ></div>`
}, true);
