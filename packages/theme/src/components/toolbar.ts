...
import { HTMLAttributes, Ref, createContext, html, useContext, virtual } from 'rezon'
...
export const ToolbarButton = virtual<ToolbarButtonProps>(({ title, children, onToggle, active, ...props }) => {
...
export const ToolbarSeparator = virtual<ToolbarSeparatorProps>((props) => (
...
export const ToolbarDropdown = virtual<ToolbarDropdownProps>(
  ({ title, ...props }) => {
    const { side } = useToolbarContext()
...
export const ToolbarSelect = virtual<ToolbarSelectProps>(
  ({ title, ...props }) => {
    const { side } = useToolbarContext()
...
export const ToolbarColorPicker = virtual<ToolbarColorPicker>(({ title, ...props }) => {
    const { side } = useToolbarContext()
  return ColorPicker({
...
const StyledToolbar = virtual<ToolbarProps>(({ mode }) => {
...
export const Toolbar = virtual<ToolbarProps>(props => {
