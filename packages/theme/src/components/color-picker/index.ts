...
import { HexColorInput, RgbaStringColorPicker } from '@/components/colorful'
import { colord } from 'colord'
import tw, { css } from 'twin.macro'
import { Button } from '../button'
import { Icon, IconCustom } from '../icon'
import { Popover, PopoverContent, PopoverContentProps, PopoverTrigger } from '@/components/popover'
...
export interface ColorPickerProps {
  ...
  renderButton?: (props: { type: 'button' | 'arrow'; children: unknown }) => unknown
...
export const ColorPicker = c<ColorPickerProps & { className?: string }>(
  (
    ...
    const renderColorButton = () => {
      return Button({
        className: css(tw`px-1.5 inline-flex flex-col`, className),
        disabled: disabled,
        type: 'text',
        onClick: () => handleSelect(value),
        children: html`
          <span class="${css(tw`inline-block`)}">${children}</span>
          <span class="${css(tw`inline-block -mt-[1px]`)}">
    ...
    const renderArrowButton = () => {
      return Button({
        ...
        className: css([tw`w-5 rounded -ml-1`, className]),
      })
    }
    ...
    return Popover({
      ...
      children: [
        PopoverTrigger({
          asChild: true,
          children: html`<div data-open=${open || undefined} class="${css([
            tw`flex rounded-md`,
            css`
                &[data-open='true'] {
                  ${tw`bg-gray-100`}
            ...
            ${when(renderButton, (render) => render({ type: 'button', children: renderColorButton() }))}
            ${when(renderButton, (render) => render({ type: 'arrow', children: renderArrowButton() }))}
          </div>`
        }),
        PopoverContent({
          align: 'start',
          side: side,
          sideOffset: 1,
          children: html`<div class=${css([
            tw`bg-white shadow-outer rounded-md text-base pt-1`,
            size === 'large' && tw`text-lg`,
          ])}>
            ${when(defaultColor, (defaultColor) => html`<div class=${css(tw`flex items-center gap-2 p-2 mb-2 cursor-pointer hover:bg-black/5`)} @click=${() => handleSelect(defaultColor.color)}>
              ${ColorPickerItem({ palette, color: defaultColor.color, activeColors: [] })}
              <span>${defaultColor.title}</span>
            </div>`)}
            <div class=${css(tw`px-2`)}>
              ${repeat(colors, (_, index) => index, (group) => ColorPickerGroup({ activeColors: [value], colors: group.map(color => (typeof color === 'string' ? color : color.color)), palette, onSelect: (value) => handleSelect(value) }))
            }
              ${when(enableRecent, () => html`<div class=${css(tw`mt-2`)}>
                <div class=${css(tw`text-base`)}>${locale.recent.title}</div>
                <div class=${css(tw`flex items-center`)}>
                  ${when(localColors.length === 0, () => html`<div class=${css(tw`text-black/10 text-base`)}>${locale.recent.empty}</div>`)}
                  ${repeat(localColors, (color) => color, (localColor) => ColorPickerItem({ palette, color: localColor, activeColors: [value], onSelect: (value) => handleSelect(value) }))
              }
                </div>
              </div>`)}
            </div>
            ${when(enableCustom, () => Popover({
                trigger: 'click',
                children: [
                  PopoverTrigger({
                    asChild: true,
                    children: html`
                  <div class=${css([
                      tw`flex justify-between items-center border-t border-black/10 p-2 mt-2 cursor-pointer hover:bg-black/5`,
                    ])}>
                    <div class=${css(tw`flex gap-1 justify-center items-center`)}>
                      ${IconCustom({
                      children: html`<svg viewBox="0 0 1600 924" width="24px" height="24px">
                          <path
                  ...
                  PopoverContent({
                    ...
                    sideOffset: 6,
                    align: 'end',
                    children: html`<div className=${css([
                    ...
                    ${HexColorInput({
                        className: css(tw`outline-none border border-gray-200 rounded bg-gray-100 text-base uppercase px-2 py-0.5 text-center`),
                        prefixed: true,
                        alpha: true,
