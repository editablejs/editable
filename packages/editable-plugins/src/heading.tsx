...
export interface HeadingOptions {
  ...
  hotkeys?: Hotkeys;
  style?: Partial<
    Record<HeadingType, Record<'fontSize' | 'fontWeight', string>>
  >;
}
...
export const HeadingEditor = {
  ...
  toggle: (
    ...
    type?: HeadingType | typeof PARAGRAPH_KEY
  ) => {
...
export const withHeading = <T extends Editable>(
    ...
    editor.normalizeSelection((selection) => {
      ...
      for (const [_, path] of lowestBlocks) {
        if (type !== PARAGRAPH_KEY) {
          const style = {
            ...defaultHeadingStyle,
            ...(HEADING_OPTIONS.get(editor) ?? {}).style,
          }[type];
  ...
  newEditor.renderElement = ({ element, attributes, children }) => {
    if (HeadingEditor.isHeading(editor, element)) {
      const Heading = HeadingTags[element.type] as
        | 'h1'
        | 'h2'
        | 'h3'
        | 'h4'
        ...
        | 'h6';
      return (
        <Heading className={`${prefixCls}`} {...attributes}>
          {children}
        </Heading>
      );
    }
  ...
  newEditor.onKeydown = (e: KeyboardEvent) => {
    ...
    if (selection && Range.isCollapsed(selection) && isHotkey('enter', e)) {
      const entry = Editor.above(newEditor, {
        match: (n) =>
          Editor.isBlock(newEditor, n) && !!n.type && n.type in HeadingTags,
      });
      if (entry) {
        const [_, path] = entry;
