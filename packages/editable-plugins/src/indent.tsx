...
type Hotkeys = Record<
  IndentPluginType,
  string | ((e: KeyboardEvent) => boolean)
>;
...
export const IndentEditor = {
  ...
  canSetIndent: (editor: IndentEditor, mode: IndentMode = 'auto') => {
    ...
    if (isCollapsed && (!selectLine || mode === 'line')) {
      ...
      if (Editor.isStart(editor, selection.focus, path) || selectLineEdge)
        return true;
    } else {
      return selectLine;
    }
...
const renderIndent = (
  ...
  if (textIndent && type === INDENT_KEY) {
    children = (
      <span
        className="editable-indent"
        style={{ width: getIndentSize(editor, 'text', textIndent) }}
      >
        {children}
      </span>
    );
...
export const withIndent = <T extends Editable>(
  editor: T,
