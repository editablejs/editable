import { Editable } from '@editablejs/editable-editor';
import classNames from 'classnames';
import React, { useCallback, useContext, useMemo } from 'react';
import { InsertAction, SplitAction } from './action';
import { TableContext } from './context';
import { Grid } from '@editablejs/editable-editor';
...
const TableRowHeaderDefault: React.FC<TableHeaderProps> = ({
  editor,
  table,
}) => {
      ...
      headers.push(
        <div
          ...
          className={classNames(
            `${prefixRowsCls}-item`,
            { [`${prefixRowsCls}-item-hover`]: hover },
            { [`${prefixRowsCls}-item-full`]: hover && selected.rowFull }
          )}
...
const TableRowHeader = React.memo(TableRowHeaderDefault, (prev, next) => {
  ...
  return (
    ...
    children.every(
      (item, index) => item.contentHeight === nextChildren[index].contentHeight
    )
  );
});

const TableColHeaderDefault: React.FC<TableHeaderProps> = ({
  editor,
  table,
}) => {
    ...
    for (let i = 0; i < colsWidth.length; i++) {
      ...
      headers.push(
        <div
          ...
          className={classNames(
            `${prefixColsCls}-item`,
            { [`${prefixColsCls}-item-hover`]: hover },
            { [`${prefixColsCls}-item-full`]: hover && selected.colFull }
          )}
