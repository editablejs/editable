...
import { Editor, Node, Range, Transforms, Point, BaseSelection } from 'slate';
...
export const ContentEditable = (props: EditableProps) => {
  ...
  const changeFocus = (focus: boolean) => {
    ...
    setBoxRects((rects) => {
      return rects.map((rect) => {
        rect.color = focus
          ? drawSelectionStyle.focusBgColor
          : drawSelectionStyle.blurBgColor;
        return rect;
      });
    });
  };
      ...
      if (count === 1 && !isCollapsed) {
        ...
        if (text) {
          ...
          Transforms.select(editor, {
            anchor: Editable.findPointOnLine(
              editor,
              focusPath,
              startOffset,
              true
  ...
  const drawCaret = (range: Range) => {
    ...
    if (
      !domRange.collapsed ||
      !isFocused ||
      (rects = domRange.getClientRects()).length === 0
    )
      return setCaretRect(null);
    const rect = Object.assign({}, rects[0].toJSON(), {
  ...
  const drawBoxs = (range: Range) => {
    ...
    setTextareaRect((rect) => {
      ...
      newRect = Object.assign({}, newRect, {
        left: newRect.left + newRect.width,
      });
  ...
  return (
    <ReadOnlyContext.Provider value={readOnly}>
      ...
      <Shadow ref={(current) => EDITOR_TO_SHADOW.set(editor, current)}>
        {drawSelection && (
          <ShadowBox
            rect={caretRect || { width: 0, height: 0, top: 0, left: 0 }}
            ref={caretRef}
            style={{
              willChange: 'opacity, transform',
              opacity: caretRect ? 1 : 0,
            }}
          />
        )}
        {drawSelection &&
          boxRects.map((rect, index) => (
            <ShadowBox
              key={index}
              rect={rect}
              style={{ willChange: 'transform', ...rect.style }}
            />
          ))}
        {
          <ShadowBox
            rect={Object.assign({}, textareaRect, {
              color: 'transparent',
              width: 1,
            })}
            style={{
