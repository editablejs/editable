...
export const Grid = {
  ...
  create: <G extends Grid, R extends GridRow, C extends GridCell>(
    ...
    const gridColsWdith: number[] =
      grid.colsWidth ?? rows[0].children.map(() => 35);
    return {
      type: 'grid',
      children: rows,
      ...grid,
  ...
  removeCol: (editor: Editable, at: GridLocation, index: number): void => {
    ...
    let newColsWidth =
      colsWidth?.concat() ?? children[0].children.map(() => 35);
    // 只有一列删除表格
  ...
  *cells(
  ...
  select: (
    editor: Editable,
    at: GridLocation,
    selection: Partial<GridSelection> = {}
  ) => {
    if (Path.isPath(at)) {
      const entry = Grid.findGrid(editor, at);
      if (!entry) return;
    ...
    const {
      start = [0, 0],
        ...
        Grid.getRowCount(editor, at) - 1,
        Grid.getColCount(editor, at) - 1,
      ],
    } = selection;
