...
export type BaseAttributes = Omit<
...
export const Editable = {
  isEditor(value: any): value is Editable {
    return (
      ...
      !!(value as Editable).onSelectionChange
    );
  },
  /**
    ...
    if (Text.isText(node)) {
      ...
      if (node.children.length === 1)
        return (
          ...
          Editable.isEmpty(editor, (node as Element).children[0])
        );
      return false;
    }
  },
  ...
  isSelectLineEdge(
    ...
    const [block] = entry;
    const rangeLines = getLineRectsByRange(editor, {
      anchor: point,
      focus: point,
    });
  ...
  toSlateNode(editor: Editable, offsetNode: DOMNode): Node {
    let domEl = isDOMElement(offsetNode)
      ? offsetNode
  ...
  findClosestPoint(
    ...
    if (Text.isText(node)) {
      ...
      const textNode = isDOMText(offsetNode)
        ? offsetNode
      ...
      if (!point)
        return Editable.toLowestPoint(editor, Editable.findPath(editor, node));
      return point;
    }
    return null;
  },
