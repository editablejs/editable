  ...
  return (
    <div css={[tw`sticky top-0 flex flex-col lg:bottom-0 lg:h-screen`, isOpen && tw`h-screen`]}>
      <nav tw="bg-wash dark:bg-wash-dark z-50 flex w-full items-center justify-between pt-0 pr-5 lg:block lg:px-5 lg:pt-4">
        <div tw="flex items-center xl:w-full xl:max-w-xs">
          <button
            type="button"
            aria-label="Menu"
            onClick={() => setIsOpen(!isOpen)}
            css={[
              tw`flex h-full items-center px-4 lg:hidden`,
              isOpen && tw`text-link dark:text-link-dark mr-0`,
            ]}
          >
          ...
          <div tw="hidden h-auto flex-initial items-center pr-5 pt-0.5 leading-loose sm:flex lg:w-full lg:pr-5">
            <div tw="bg-highlight dark:bg-highlight-dark text-link dark:text-link-dark mb-px whitespace-nowrap rounded px-1 text-xs font-bold uppercase tracking-wide">
              Beta
            </div>
          </div>
          <div tw="block dark:hidden">
            <button
              type="button"
        ...
        <div tw="my-4 mx-0 flex h-10 w-full justify-end lg:hidden lg:max-w-sm">
          ...
          <button
            ...
            type="button"
            css={[
              tw`relative top-px ml-4 inline-flex items-center rounded-full px-1.5 lg:ml-6 lg:hidden`,
              showFeedback && tw`bg-card dark:bg-card-dark`,
            ]}
          ...
          <div tw="block dark:hidden">
            <button
              ...
              tw="ml-4 flex h-full items-center p-1 lg:ml-6 lg:hidden"
            >
              {darkIcon}
            </button>
          </div>
          <div
            ...
            css={[tw`fixed top-12 right-0`, showFeedback && tw`block`, !showFeedback && tw`hidden`]}
          >
            <Feedback
              onSubmit={() => {
                clearTimeout(feedbackAutohideRef.current)
...
function TabButton({
  ...
  const classes = [
    tw`inline-flex items-center w-full border-b-2 justify-center text-base leading-9 px-3 pb-0.5 hover:text-link hover:bg-gray-50`,
    isActive && tw`text-link dark:text-link-dark dark:border-link-dark border-link font-bold`,
    !isActive && tw`border-transparent`,
  ]
  return (
    <button css={classes} onClick={onClick}>
      {children}
    </button>
  )
}
